...
class Box(BasePrimitive):
    ...
    def __init__(self, length: float, width: float, height: float, custom_data: Optional[Any] = None, mode: Mode = Mode.ADD):
        self.length = length
        self.width = width
        self.height = height
    ...
    def create_part(self) -> Part:
        return BoxSetPart(size=Vector((self.length, self.width, self.height)), custom_data=self.custom_data)
