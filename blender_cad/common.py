...
if TYPE_CHECKING:
    from .location import Location, CurveLocation, Transform
...
VectorLike = Union[Vector, tuple[Union[float, int]], list[Union[float, int]], float, int, 'Axis', 'Transform']
...
PartLike = Union['Part', 'BuildPart', 'BaseCurve', 'BuildCurve', 'curve', 'chain', 'ml', 'Joint']
...
def match_tags(applied_tags: Iterable[str], target_tags: Iterable[str], invert: bool = False) -> bool:
    if not target_tags:
        return invert
    ...
    for target in target_tags:
        if '*' in target:
            if any(fnmatch.fnmatchcase(applied_tag, target) for applied_tag in applied_tags):
                has_match = True
                break
        else:
