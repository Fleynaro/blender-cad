...
from .common import AbstractCurve
from .curve import BuildCurve, Curve, Polyline, Spline, make_curve
from .location import Location, Pos, Scale, SurfaceLocation
...
class GeometryEntity(ABC):
    ...
    @abstractmethod
    def _add_tags(self, tags: Iterable[str]):
        raise NotImplementedError

    @abstractmethod
    def _remove_tags(self, tags: Iterable[str]):
        raise NotImplementedError
    ...
    def owner_loc(self, loc: Union["Location", "Vector"]) -> "Location":
        ...
        scaled_local_matrix = (Scale(self.source_part.scale) * local_loc).matrix
        return Location(scaled_local_matrix, parent_loc=self.source_part.loc)
            ...
            Face(
                ...
                wires=[
                    Wire(
                        self._owner,
                        edges=[
                            Edge(self._owner, edges=[e], start_v=e.verts[0])
                            for e in f.edges
                        ],
                    )
                ],
            )
            for f in self.bm_faces()
        )
    ...
    def all_single_edges(self) -> "ShapeList[Edge]":
        ...
        return ShapeList(
            Edge(self._owner, edges=[e], start_v=e.verts[0]) for e in self.bm_edges()
        )
...
class Vertex(GeometryEntity):
    ...
    @property
    def tags(self):
        return self._owner.part._get_tags(
            "POINT",
            [self.element.index],
        )
    ...
    @override
    def is_new(self, checkpoint: Optional["GeometryCheckpoint"] = None) -> bool:
        cp = checkpoint or self._owner.part._last_op_checkpoint
        if not cp:
            return False
...
class Edge(GeometryEntity, AbstractCurve):
    """
    ...
    def __init__(
        self, owner: "Topology", edges: list["BMEdgeWrapper"], start_v: "BMVertWrapper"
    ):
        super().__init__(owner)
        self.elements = edges
    ...
    def tags(self):
        return self._owner.part._get_tags(
            "EDGE",
            [e.index for e in self.elements],
        )
    ...
    def center(self) -> "Location":
        ...
        return self.owner_loc(
            sum([e.center for e in self.elements], Vector()) / len(self.elements)
        )
    ...
    def at(self, t: Optional[float] = None, t_m: Optional[float] = None) -> Location:
        ...
        return self._at_dist(
            (self.length() * max(0.0, min(1.0, t))) if t is not None else t_m
        )
...
class Wire(Edge):
    ...
    def __init__(self, owner: "Topology", edges: list[Edge], is_outer: bool = True):
        raw_edges: list[BMEdgeWrapper] = []
        for e in edges:
            raw_edges.extend(e.elements)

        # Determine vertex frequencies to safely detect endpoints of an open wire
        vert_counts = {}
        for re in raw_edges:
            for v in re.verts:
                vert_counts[v] = vert_counts.get(v, 0) + 1

        start_verts = [v for v, count in vert_counts.items() if count == 1]
        remaining = set(raw_edges)
        ordered_elements = []

        if start_verts:
            # Open wire: start at one of the free endpoints
            curr_v = start_verts[0]
            curr_e = next(re for re in remaining if curr_v in re.verts)
        else:
            # Closed wire loop: fall back to the first provided element
            if edges and edges[0].elements:
                curr_v = edges[0].start_v
                curr_e = edges[0].elements[0]
            else:
                curr_v = None
                curr_e = None
        ...
        # Sequentially sort elements into a non-disjoint continuous chain
        while curr_e is not None:
            ordered_elements.append(curr_e)
            remaining.remove(curr_e)
            curr_v = curr_e.other_vert(curr_v)

            next_e = None
            for re in remaining:
                if curr_v in re.verts:
                    next_e = re
                    break
            curr_e = next_e

        # Call Edge.__init__ with the continuous chain and correctly resolved start_v
        super().__init__(owner, ordered_elements, start_v)
        self._edges = edges
    ...
    def vertices(self) -> ShapeList[Vertex]:
        ...
        res = []
        curr_v = self.start_v
        res.append(Vertex(self._owner, curr_v))

        for e in self.elements[:-1]:
            curr_v = e.other_vert(curr_v)
            res.append(Vertex(self._owner, curr_v))

        # Closure check to avoid appending duplicated start vertex in closed loops
        last_v = self.elements[-1].other_vert(curr_v)
        if last_v != self.start_v:
            res.append(Vertex(self._owner, last_v))
...
class UVSelector:
    ...
    def set_m(
        self, u: float = 0.0, v: float = 0.0, projection: Optional[UVProjection] = None
    ):
        """Fixes the UV coordinate to absolute metric values (relative to 0,0)."""
        return self._clone(
            u_fixed=0.0, v_fixed=0.0, u_offset_m=u, v_offset_m=v, projection=projection
        )
    ...
    def min_x(self):
        return self._clone(
            criteria=self.criteria + [(lambda loc: loc.position.x, False)]
        )

    def max_x(self):
        return self._clone(
            criteria=self.criteria + [(lambda loc: loc.position.x, True)]
        )

    def min_y(self):
        return self._clone(
            criteria=self.criteria + [(lambda loc: loc.position.y, False)]
        )

    def max_y(self):
        return self._clone(
            criteria=self.criteria + [(lambda loc: loc.position.y, True)]
        )

    def min_z(self):
        return self._clone(
            criteria=self.criteria + [(lambda loc: loc.position.z, False)]
        )

    def max_z(self):
        return self._clone(
            criteria=self.criteria + [(lambda loc: loc.position.z, True)]
        )
    ...
    def select(
        self, at_uv_fn: Callable[[float, float, UVProjection], Location]
    ) -> "Location":
        """
        ...
        for key_func, reverse in self.criteria:
            actual_key_func: Callable[[Location], float] = (
                (lambda loc: key_func(loc.local)) if self.is_local else key_func
            )
            # Sort to find the "best" value (min or max)
            current_pool.sort(
                key=lambda item: actual_key_func(item[0]), reverse=reverse
            )
        ...
        current_pool.sort(
            key=lambda item: (item[1] - self.pref_u) ** 2 + (item[2] - self.pref_v) ** 2
        )
        ...
        if (
            ...
            and self.u_offset_m == 0.0
            and self.v_offset_m == 0.0
        ):
        ...
        return (
            at_uv_fn(
                offset_u, offset_v, self.u_offset_m, self.v_offset_m, self.projection
            )
            * self.final_offset
        )
...
class Face(GeometryEntity):
    ...
    def __init__(
        self, owner: "Topology", faces: list["BMFaceWrapper"], wires: list[Wire]
    ):
        super().__init__(owner)
        self.elements = faces
        self._wires = wires
        ...
        self._best_uv_proj: Optional[
            tuple[UVProjection, list[Vector], dict[int, int]]
        ] = None
        self._last_uv_proj: Optional[
            tuple[UVProjection, list[Vector], dict[int, int]]
        ] = None
    ...
    def tags(self):
        return self._owner.part._get_tags(
            "FACE",
            [f.index for f in self.elements],
        )
    ...
    def mat(self, material: Optional["MaterialLayer"]):
        self._owner.part._set_material(material, faces=self.elements)
    ...
    def center(self) -> Location:
        return self.owner_loc(
            sum([f.center for f in self.elements], Vector()) / len(self.elements)
        )
    ...
    @override
    def vertices(self) -> ShapeList[Vertex]:
        return ShapeList({v for e in self.edges() for v in e.vertices()})
    ...
    def part(self) -> "Part":
        ...
        with BuildPart(mode=Mode.PRIVATE) as bp:
            ...
            # 2. Transfer all material slots from the original object to the new mesh
            for slot in self.source_part.obj.material_slots:
                dest_part.obj.data.materials.append(slot.material)

            dest_bm = dest_part._ensure_bmesh(write=True)
            ...
            for f in self.elements:
                ...
                new_face = dest_bm.native.faces.new(new_verts)

                # 3. Transfer the original material index to the newly created face
                new_face.material_index = f.native.material_index

            dest_part._write_bmesh()
        return bp.part
    ...
    def _detect_geom_type(self) -> tuple[GeomType, Vector]:
        ...
        if sum(sphere_score) / len(sphere_score) > 0.98:
            ...
            axis = Vector((0, 0, 1))
            verts = sorted(
                self.bm_verts(), key=lambda v: len(v.link_edges), reverse=True
            )
            if len(verts) >= 2:
                pole1 = verts[0].co
                pole2 = verts[1].co
    ...
    def _detect_best_uv_projection(self):
        ...
        if self._best_uv_proj is None:
            ...
            for p_type in UVProjection:
                ...
                score = get_projection_score(
                    verts_co, norm_uvs, self.elements, v_indices, p_type, self.main_axis
                )
                if score < min_score:
    ...
    def location(self, selector: Optional["UVSelector"] = None) -> SurfaceLocation:
    ...
    def _at_uv(
        ...
        u_offset_m: float = 0.0,
        v_offset_m: float = 0.0,
        projection: UVProjection | None = None,
    ) -> Location:
        ...
        if v_offset_m != 0.0:
            res_u, res_v = self._walk_metric(
                res_u, res_v, v_offset_m, False, projection, uvs, v_indices
            )
        if u_offset_m != 0.0:
            res_u, res_v = self._walk_metric(
                res_u, res_v, u_offset_m, True, projection, uvs, v_indices
            )
    ...
    def _walk_metric(
        ...
        obj_scale = self._owner.part.scale
        ...
        for _ in range(500):  # Safety limit for very dense meshes
            if rem_dist < 1e-7 or not current_hit:
                break
            ...
            for i in range(3):
                intersect = geometry.intersect_line_line_2d(
                    curr_uv, curr_uv + move_dir_uv, uv_tri[i], uv_tri[(i + 1) % 3]
                )
                if intersect:
            ...
            if edge_idx == -1:
                ...
                current_hit = self._find_face_and_uv_tri(
                    curr_uv, projection, uvs, v_indices
                )
                continue
            ...
            if dist_to_edge_m >= rem_dist:
                ...
                current_hit = self._find_next_triangle(
                    v1, v2, bm_face, curr_uv, projection, uvs, v_indices
                )

        return curr_uv.x % 1.0, curr_uv.y  # Wrap X for cylindrical logic

    def _iter_face_triangles(
        self,
        face: "BMFaceWrapper",
        projection: UVProjection,
        uvs: list[Vector],
        v_indices: dict,
    ) -> Iterable[tuple[tuple["BMVertWrapper"], list[Vector]]]:
    ...
    def _find_face_and_uv_tri(
        self,
        target_uv: Vector,
        projection: UVProjection,
        uvs: list[Vector],
        v_indices: dict,
        faces: list["BMFaceWrapper"] = None,
        ...
        for f in search_set:
            for v_tri, uv_tri in self._iter_face_triangles(
                f, projection, uvs, v_indices
            ):
                ...
                if use_closest:
                    cp = geometry.closest_point_on_tri(
                        target_uv.to_3d(),
                        uv_tri[0].to_3d(),
                        uv_tri[1].to_3d(),
                        uv_tri[2].to_3d(),
                    ...
                    if dist < min_dist_2d:
                        ...
                        closest_data = (
                            ...
                            f,
                            uv_tri,
                            get_barycentric_2d(cp.to_2d(), *uv_tri),
                        )
        return closest_data

    def _find_next_triangle(
        self,
        v1: "BMVertWrapper",
        v2: "BMVertWrapper",
        current_face: "BMFaceWrapper",
        target_uv: Vector,
        projection: UVProjection,
        uvs: list[Vector],
        v_indices: dict,
        ...
        # 1. Same face (n-gons)
        hit = self._find_face_and_uv_tri(
            target_uv, projection, uvs, v_indices, [current_face]
        )
        if hit:
            return hit
        ...
        for faces in [common_faces, fallback_faces]:
            for f in faces:
                if f != current_face and f in self.elements:
                    hit = self._find_face_and_uv_tri(
                        target_uv, projection, uvs, v_indices, [f]
                    )
                    if hit:
                        return hit
        return None

    def _calc_uv_location(
        self,
        u: float,
        v: float,
        projection: UVProjection,
        ...
        hit = self._find_face_and_uv_tri(
            Vector((u, v)), projection, uvs, v_indices, use_closest=True
        )
...
class Topology:
    ...
    def __init__(
        self, bmesh_wrapper: "BMeshWrapper", part: "Part", config=TopologyConfig()
    ):
    ...
    def _build_topology(self):
        ...
        # 1. Process surface topology (Faces, Wires, and boundary Edges)
        ...
        loose_wires = self._process_loose_wires()
        loose_verts = self._process_loose_vertices()

        # 2. Wires
        self.wires = list({w for f in self.faces for w in f.wires()}) + loose_wires

        # 3. Edges
        self.edges = list({w for f in self.wires for w in f.edges()})

        # 3. Vertices
        self.vertices = (
            list({v for e in self.edges for v in e.vertices()}) + loose_verts
        )

    def _process_loose_wires(self) -> list[Wire]:
        """Finds, chains, and segments all BMesh edges that have no linked faces,
        wrapping them into CAD Wires.
        """
        loose_bmesh_edges = [e for e in self.bw.edges if not e.link_faces]
        loose_wires: list[Wire] = []

        while loose_bmesh_edges:
            # Find the best starting edge and its correct orientation vertex
            start_e, start_v = self._find_best_loose_start(loose_bmesh_edges)
            loose_bmesh_edges.remove(start_e)
            chain = [start_e]

            curr_v = start_e.other_vert(start_v)

            # Trace the continuous line/polyline of loose edges
            while True:
                next_e = None
                for e in curr_v.link_edges:
                    if e in loose_bmesh_edges:
                        next_e = e
                        break
                if next_e:
                    loose_bmesh_edges.remove(next_e)
                    chain.append(next_e)
                    curr_v = next_e.other_vert(curr_v)
                else:
                    break

            # Segment the identified loose chain based on sharp angle thresholds
            segmented_edges = self._segment_loop(chain, start_v)

            # Package the segmented loose edges into a CAD Wire.
            # is_outer=False because a loose wire doesn't define a face outer boundary.
            loose_wire = Wire(self, segmented_edges, is_outer=False)
            loose_wires.append(loose_wire)

        return loose_wires

    def _find_best_loose_start(
        self, loose_edges: list["BMEdgeWrapper"]
    ) -> tuple["BMEdgeWrapper", "BMVertWrapper"]:
        """Finds the optimal starting edge and vertex for a loose wire.
        1. Prefers dead-ends (open wires).
        2. For closed loops, finds a sharp corner to hide the loop seam.
        """
        # 1. Search for open endpoints (dead-ends)
        for e in loose_edges:
            for v in e.verts:
                # If a vertex is connected to only one loose edge, it's a tip
                loose_connections = sum(1 for le in v.link_edges if not le.link_faces)
                if loose_connections == 1:
                    return e, v

        # 2. Closed loops fallback: look for a sharp angle corner.
        # This prevents breaking a perfectly smooth curve and places the seam at an actual sharp corner.
        for e in loose_edges:
            for v in e.verts:
                adjacent_loose = [
                    le for le in v.link_edges if not le.link_faces and le != e
                ]
                if adjacent_loose:
                    other_e = adjacent_loose[0]

                    # Compute vectors pointing AWAY from the shared vertex
                    v1 = (e.other_vert(v).co - v.co).normalized()
                    v2 = (other_e.other_vert(v).co - v.co).normalized()
                    dot = max(-1.0, min(1.0, v1.dot(v2)))
                    angle = math.acos(dot)

                    # If deviation from a straight line exceeds threshold, use this corner as the seam
                    if abs(math.pi - angle) > math.radians(
                        self.config.edge_break_angle
                    ):
                        return e, v

        # 3. Absolute fallback: perfectly smooth closed loop (e.g., a circle).
        # A seam generation is inevitable here, but it will be the only split point.
        return loose_edges[0], loose_edges[0].verts[0]

    def _process_loose_vertices(self) -> list[Vertex]:
        """Finds all BMesh vertices with no edge connections and wraps them into CAD Vertices."""
        loose_verts = [v for v in self.bw.verts if not v.link_edges]
        return [Vertex(self, v) for v in loose_verts]

    def _grow_face_group(
        self, start_face: "BMFaceWrapper", visited_faces: set["BMFaceWrapper"]
    ):
    ...
    def _segment_loop(
        self, loop: list["BMEdgeWrapper"], wire_start_v: "BMVertWrapper"
    ) -> list[Edge]:
        ...
        for i in range(1, len(loop)):
            ...
            common_v = next(iter(set(e_prev.verts) & set(e_curr.verts)))
...
class GeometryCheckpoint:
    ...
    def __init__(self, bm: "BMeshWrapper"):
        ...
        self._cache_res: tuple[
            set[BMVertWrapper], set[BMEdgeWrapper], set[BMFaceWrapper]
        ] = (set(), set(), set())
    ...
    def get_new_entities(self, bm: "BMeshWrapper"):
        ...
        for f in bm.faces_raw:
            is_old = False
            if bvh:
                loc, _norm, idx, dist = bvh.find_nearest(f.center)
...
def _estimate_geometry_center(
    local_verts: list[Vector], p_type: UVProjection
) -> Vector:
...
def get_projection_score(
    verts_co: list[Vector],
    uvs: list[Vector],
    test_faces: list["BMFaceWrapper"],
    v_indices: dict[int, int],
    p_type: UVProjection,
    avg_norm: Vector,
):
    ...
    for f in test_faces:
        ...
        for edge in f.edges:
            ...
            if (
                p_type in [UVProjection.SPHERICAL, UVProjection.CYLINDRICAL]
                and abs(delta_uv.x) > 0.5
            ):
