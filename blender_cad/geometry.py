...
class GeometryEntity(ABC):
    ...
    def add_tags(self, *tags: str | Iterable[str]):
        if len(tags) == 1 and not isinstance(tags[0], str) and isinstance(tags[0], Iterable):
...
class Topology:
    ...
    def _build_topology(self):
        ...
        # 2. Wires
        self.wires = list(set([w for f in self.faces for w in f.wires()])) + loose_wires

        # 3. Edges
        self.edges = list(set(w for f in self.wires for w in f.edges()))

        # 3. Vertices
        self.vertices = list(set(v for e in self.edges for v in e.vertices())) + loose_verts
    ...
    def _find_best_loose_start(self, loose_edges: List['BMEdgeWrapper']) -> tuple['BMEdgeWrapper', 'BMVertWrapper']:
        ...
        for e in loose_edges:
            for v in e.verts:
                adjacent_loose = [le for le in v.link_edges if not le.link_faces and le != e]
                if adjacent_loose:
