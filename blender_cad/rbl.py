...
class RuntimeContext:
    ...
    _current_context: ContextVar[Optional["RuntimeContext"]] = ContextVar(
    ...
    def set_local_transform(
        ...
        if isinstance(target.part, Object):
            ...
            target.part.transform = (
                Pos(nx, ny, nz) * Rot(nrx, nry, nrz) * Scale(nsx, nsy, nsz)
            )
    ...
    def resolve_scalar(
        self, scalar: ScalarLike, current_target_element: "RuleBasedLayout"
    ) -> float:
        if callable(scalar):
    ...
    def resolve_position(
        self, target: PosNodeLike, current_target_element: "RuleBasedLayout"
    ) -> Vector:
        if callable(target):
            self._current_target_element = current_target_element
    ...
    def resolve_node_position(self, target: "RuleBasedLayout"):
        ...
        return sum(
            [self.get_global_transform(l).position for l in phys_elements], Vector()
        ) / len(phys_elements)

    def get_part_data(
        self, target: "RuleBasedLayout", target_shell_selector: Optional[str] = None
    ) -> PartData:
...
class DefaultRuntimeContext(RuntimeContext):
    ...
    def _init(
        ...
        parent: Optional["RuleBasedLayout"] = None,
    ):
    ...
    def set_local_transform(
        ...
        self._local_transforms[target_id] = (
            Pos(nx, ny, nz) * Rot(nrx, nry, nrz) * Scale(nsx, nsy, nsz)
        )
...
class Rule:
    ...
    class Scope(Enum):
        ...
        EACH_CHILD_WITH_SELF = (
            auto()
        )  # Apply to immediate children (1st level) including the root itself
        DEEP_PHYSICAL = auto()  # Apply recursively only to physical parts
        DEEP_PHYSICAL_WITH_SELF = (
            auto()
        )  # Apply recursively to all physical parts including the PHYSICAL root itself
        DEEP_ALL = auto()  # Apply recursively to all nodes
        DEEP_ALL_WITH_SELF = (
            auto()
        )  # Apply recursively to all nodes including the root itself

    def __init__(
        self,
        priority: float = 1.0,
        init_priority: int = 0,
        scope: Scope = Scope.DEEP_PHYSICAL_WITH_SELF,
    ):
    ...
    def on_each(self, include_self: bool = False) -> Self:
        ...
        new_rule = self.clone()
        new_rule.scope = (
            Rule.Scope.EACH_CHILD_WITH_SELF if include_self else Rule.Scope.EACH_CHILD
        )
        return new_rule
    ...
    def on_deep_physical(self, include_self: bool = False) -> Self:
        ...
        new_rule = self.clone()
        new_rule.scope = (
            Rule.Scope.DEEP_PHYSICAL_WITH_SELF
            if include_self
            else Rule.Scope.DEEP_PHYSICAL
        )
        return new_rule
    ...
    def on_deep_all(self, include_self: bool = False) -> Self:
        ...
        new_rule = self.clone()
        new_rule.scope = (
            Rule.Scope.DEEP_ALL_WITH_SELF if include_self else Rule.Scope.DEEP_ALL
        )
        return new_rule
    ...
    def __ror__(
        self, other: Union["RuleBasedLayout.Query", PartLike]
    ) -> "RuleBasedLayout":
        ...
        if isinstance(other, RuleBasedLayout.Query):
            return RuleBasedLayout(
                children=[other], rules=[self], tag=RuleBasedLayout.TAG_TAGGED
            )
        return RuleBasedLayout(rules=[self], part=other)

    def initialize(
        self, context: RuntimeContext, elements: list["RuleBasedLayout"]
    ) -> None:
        raise NotImplementedError
...
class TransformRule(Rule):
    ...
    def _eval(self, context: RuntimeContext, elements: list["RuleBasedLayout"]):
        for e in elements:
            ...
            resolved = {
                k: (v() if callable(v) else v) for k, v in self.transforms.items()
            }
            if self.local:
...
class CollisionRule(Rule):
    ...
    def shell_override(self) -> Optional["RuleBasedLayout"]:
        ...
        if "shell_override" not in data:
            data["shell_override"] = (
                ctx.resolve_target(self._shell_override)
                if self._shell_override
                else None
            )
    ...
    def _get_part_data(
        self, context: RuntimeContext, obj: "RuleBasedLayout"
    ) -> PartData:
    ...
    def _check_directed_collision(
        ...
        for v in verts_a:
            ...
            elif self.mode == self.Mode.INSIDE:  # noqa: SIM114
                if signed_dist > -self.margin:
                    violation = signed_dist + self.margin
                    s.aim(self.priority * (violation**2))

            elif self.mode == self.Mode.INTERSECT:  # noqa: SIM102
                if signed_dist > -self.margin:
                    violation = signed_dist + self.margin
                    s.aim(self.priority * (violation**2))
    ...
    def evaluate(
        ...
        for e in elements:
            if "t" in context.get_dofs(e) and context.get_dofs(e)["t"].enabled:
                ...
                context.set_global_transform(
                    e, x=eval_pos.x, y=eval_pos.y, z=eval_pos.z
                )
    ...
    def evaluate(
        ...
        for e in elements:
            ...
            if is_soft:
                s.aim_equal(
                    e_tr.rotation_loc,
                    Rot(X=target_rx, Y=target_ry, Z=target_rz),
                    k=self.priority,
                )
            else:
                context.set_global_transform(
                    e, rx=target_rx, ry=target_ry, rz=target_rz
                )
...
class SizeRule(Rule):
    ...
    def _eval(
        ...
        context: RuntimeContext,
        elements: list["RuleBasedLayout"],
        s: Optional[Solver.Session] = None,
    ) -> None:
        is_soft = context.force_soft_rules or self.soft
        for e in elements:
            transform_updates = {}
            ...
            for i, t_size_scalar in enumerate([self.x, self.y, self.z]):
                ...
                if is_soft:
                    ...
                    transform_updates[["sx", "sy", "sz"][i]] = (
                        t_size_scalar
                        / e_orig_size[i]
                        * (1.0 if e_size[i] > 0.0 else -1.0)
                    )
...
class StackRule(Rule):
    ...
    def __init__(
        ...
        gap: Union[float, tuple[float, float], Callable[[int], float]] = 1.0,
    ):
    ...
    def initialize(
        ...
        for e in elements:
            ...
            if isinstance(self.gap, tuple):
                ...
                context.get_dofs(e)["gap"] = Dof(
                    (g_min + g_max) / 2.0, min=g_min, max=g_max, inited=True
                )
    ...
    def evaluate(
        ...
        for e in elements:
            ...
            for idx, phys_elem in enumerate(e._get_phys_elements_deep()):
                ...
                context.set_local_transform(
                    phys_elem, x=offset.x, y=offset.y, z=offset.z
                )

                # Resolve the gap step based on its configuration
                if isinstance(self.gap, tuple):
                    step_gap = (
                        context.get_dofs(e)["gap"].value
                        if "gap" in context.get_dofs(e)
                        else (self.gap[0] + self.gap[1]) / 2.0
                    )
...
class RuleBasedLayout:
    ...
    class QueryFilterChain(Query):
        ...
        def filter(self, callback: Callback) -> "RuleBasedLayout.QueryFilterChain":
            return RuleBasedLayout.QueryFilterChain(
                filter_chain=self.filter_chain + [callback], root=self.root
            )
    ...
    def __init__(
        ...
        self._children = [
            child
            if isinstance(child, (RuleBasedLayout, RuleBasedLayout.Query))
            else RuleBasedLayout.object(child)
    ...
    def clone(self) -> Self:
        return RuleBasedLayout(
            self._children, self.rules, self.part, self.id, self.tags
        )

    def __or__(
        self, other: Union[Rule, Iterable[Rule], PartNodeLike, None]
    ) -> "RuleBasedLayout":
        if other is None:
            return self
        if isinstance(other, Rule):
        ...
        if isinstance(other, Iterable):
            ...
            assert all(isinstance(r, Rule) for r in other_rules), (
    ...
    def object(part: PartLike, tag: str | Iterable[str] = []) -> "RuleBasedLayout":
        ...
        return RuleBasedLayout(
            part=part, tag=tag_to_list(tag) + [RuleBasedLayout.TAG_OBJECT]
        )
    ...
    def group(
        *args: PartNodeLike,
        ...
        return RuleBasedLayout(
            children=list(_flatten_items(args)),
            part=part,
            tag=tag_to_list(tag) + [RuleBasedLayout.TAG_GROUP],
        )
    ...
    def get_transform(elem: "RuleBasedLayout", local: bool = False):
        ...
        ctx = RuntimeContext.get_current()
        return (
            ctx.get_local_transform(elem) if local else ctx.get_global_transform(elem)
        )
    ...
    def get_size(elem: "RuleBasedLayout", local: bool = False):
        ...
        ctx = RuntimeContext.get_current()
        return (
            ctx.get_part_data(elem).part.orig_size
            * RuleBasedLayout.get_transform(elem, local).scale
        )
    ...
    def look_along(direction: VectorLike, soft: bool = False) -> Rule:
        ...
        return RuleBasedLayout.look_at(
            lambda: (
                RuleBasedLayout.get_transform(
                    RuleBasedLayout.current_target_element()
                ).position
                + extract_vector(direction)
            ),
            soft=soft,
        )
    ...
    def size(
        ...
        return SizeRule(x=x, y=y, z=z, soft=soft, local=local, init_only=init)

    @staticmethod
    def grow(size=10e3) -> Rule:
        """Applies a grow soft constraint to a layout element."""
        return RuleBasedLayout.size(x=size, y=size, z=size, soft=True).with_priority(
            100.0 / (size**2)
        )
    ...
    @staticmethod
    def gravity(
        target: PosNodeLike, pull: Optional[bool] = None, push: Optional[bool] = None
    ) -> Rule:
    ...
    @staticmethod
    def root(
        node: Optional[Union["RuleBasedLayout", Query]] = None,
    ) -> "RuleBasedLayout.QueryFilterChain":
        return RuleBasedLayout.QueryFilterChain(
            root=node and RuleBasedLayout._from(node)
        )
    ...
    @staticmethod
    def tagged(
        *tags: str, root: Optional[Union["RuleBasedLayout", Query]] = None
    ) -> "RuleBasedLayout.QueryFilterChain":
    ...
    def _get_phys_elements_deep(
        self, seen: Optional[set["RuleBasedLayout"]] = None
    ) -> list["RuleBasedLayout"]:
            ...
            for rule in child.rules:
                active_bindings.append(
                    RuleBinding(
                        rule,
                        child,
                        list(dict.fromkeys(child._resolve_rule_targets(rule))),
                    )
                )
    ...
    def resolve(
        self, solver: SolverLike = Solver(), mode=Mode.ADD, instantiation_delay_sec=0.1
    ):
        ...
        with context:
            ...
            for s in solver:
                for dof_map in context._dofs.values():
                    for dof in dof_map.values():
                        if dof.enabled:
                            # Feed back mutations downstream from optimizer iterations
                            dof.value = s.param(
                                init_value=dof.value, min=dof.min, max=dof.max
                            )
