...
from .location import Location, Scale, TransformExpr, VectorLike
...
class BuildPart:
    ...
    def _get_context(cls) -> "BuildPart":
        ...
        if not cls._context_stack:
            raise RuntimeError(
                "Operation must be performed within a 'with BuildPart()' context"
            )
...
def set_tags(
    tags: Iterable[str], domain: Optional[Literal["FACE", "EDGE", "POINT"]] = None
):
