...
class ProportionalEdit(ABC):
    ...
    def mix(self, other: "ProportionalEdit", factor: Union["ProportionalEdit", float]):
        ...
        return LambdaPropEdit(
            func=lambda p, ctx: (lambda a, b, f: a + (b - a) * f)(  # noqa: PLC3002
                self.calculate_weight(p, ctx),
    ...
    def clamp(self, min_val: float = 0.0, max_val: float = 1.0):
        ...
        return LambdaPropEdit(
            func=lambda p, ctx: max(
                min_val, min(max_val, self.calculate_weight(p, ctx))
            ),
        )
...
class RadialPropEdit(ProportionalEdit):
    ...
    def __init__(
        self,
        origin: Union[Vector, Location] = Vector((0, 0, 0)),
        radius: float = 1.0,
...
class LinearPropEdit(ProportionalEdit):
    ...
    def __init__(
        self, axis: Union[Axis, Vector] = Axis.Z, falloff: Falloff = Falloff.SMOOTH
    ):
...
def _apply_transform(
    ...
    if prop_edit is None:
        ...
        bmesh.ops.transform(bm, verts=verts, matrix=op.matrix, space=space.matrix)
    else:
...
def subdivide(
    ...
    bmesh.ops.subdivide_edges(
        bm.native, edges=[e.native for e in edges], cuts=cuts, use_grid_fill=True
    )
...
def extrude(
    ...
    new_geometry: list[
        Union[bmesh.types.BMFace, bmesh.types.BMEdge, bmesh.types.BMVert]
    ] = []
    source_natives = []
    ...
    if entity_type == "Face":
        ...
        res = bmesh.ops.extrude_face_region(bm.native, geom=[f.native for f in faces])
        ...
        new_side_faces = [
            f
            for f in all_linked_faces
            if f not in source_natives and f not in new_geometry
        ]
        ...
        res = bmesh.ops.extrude_edge_only(bm.native, edges=[e.native for e in edges])
        new_geometry = res["geom"]
        ...
        res = bmesh.ops.extrude_vert_indiv(bm.native, verts=[v.native for v in verts])
    ...
    if delete_source and source_natives:
        bmesh.ops.delete(
            bm.native,
            geom=source_natives,
            context=("FACES" if entity_type == "Face" else "EDGES"),
        )
    ...
    part._add_tags(
        "POINT",
        [v.index for v in new_geometry if isinstance(v, bmesh.types.BMVert)],
        tags,
    )
    part._add_tags(
        "EDGE",
        [e.index for e in new_geometry if isinstance(e, bmesh.types.BMEdge)],
        tags,
    )
    part._add_tags(
        "FACE",
        [f.index for f in new_geometry if isinstance(f, bmesh.types.BMFace)],
        tags,
    )
...
def _copy_faces_to_bmesh(
    ...
    for face in faces:
        for v in face.verts:
            vert_map.setdefault(v, dst_bm.verts.new(v.co.copy()))

    result = []
    for face in faces:
        try:
            result.append(dst_bm.faces.new([vert_map[v] for v in face.verts]))
        except ValueError:
            pass
...
def solidify_faces(
    ...
    dup_geom = bmesh.ops.duplicate(bm.native, geom=src_faces)["geom"]

    new_faces = [g for g in dup_geom if isinstance(g, bmesh.types.BMFace)]
    ...
    bmesh.ops.delete(bm.native, geom=new_faces, context="FACES")

    # Optional shell offset
    _move_verts_along_normals(list({v for f in clip_faces for v in f.verts}), offset)

    # Extrude whole region
    extruded_geom = bmesh.ops.extrude_face_region(clip_bm, geom=clip_faces)["geom"]
    original_verts = {v for f in clip_faces for v in f.verts}
    move_verts = [
        v
        for v in extruded_geom
        if isinstance(v, bmesh.types.BMVert) and v not in original_verts
    ]

    # Move new cap
    _move_verts_along_normals(move_verts, height)

    # Fix normals
    bmesh.ops.recalc_face_normals(clip_bm, faces=list(clip_bm.faces))
...
def delete(entities: GeometryEntityLike = None):
    ...
    if geom_to_delete:
        bmesh.ops.delete(bm.native, geom=geom_to_delete, context=del_context)
...
def bevel(entities: GeometryEntityLike = None, radius: float = 0.1, segments: int = 10):
    ...
    mod: bpy.types.BevelModifier = part.obj.modifiers.new(
        name="BP_Bevel_Op", type="BEVEL"
    )
...
def mirror(axis: Axis = Axis.X):
    ...
    mod: bpy.types.MirrorModifier = part.obj.modifiers.new(
        name="BP_Mirror_Op", type="MIRROR"
    )
...
def simple_deform(
    ...
    mod: bpy.types.SimpleDeformModifier = part.obj.modifiers.new(
        name="BP_Deform_Op", type="SIMPLE_DEFORM"
    )
...
def _apply_simple_deform_op(
    ...
    axis_mapping = {Axis.X: Axis.X, Axis.Y: Axis.Z, Axis.Z: Axis.Y}
...
def wrap(
    ...
    *,
    ...
    mod: bpy.types.ShrinkwrapModifier = part.obj.modifiers.new(
        name="BP_Wrap_Op", type="SHRINKWRAP"
    )
...
def _apply_boolean(target: bpy.types.Object, tool: bpy.types.Object, mode: Mode):
    ...
    mod: bpy.types.BooleanModifier = target.modifiers.new(
        name="TempBool", type="BOOLEAN"
    )
    mod.object = tool
...
def add(
    ...
    need_copy = (_make_copy or len(active_locs) > 1) and not isinstance(
        to_add, (BuildCurve, BaseCurve)
    )
    for loc in active_locs:
        ...
        if isinstance(part, BoxSetPart):
            ...
            if not cloned_part.empty:
                if mode == Mode.JOIN or (
                    mode == Mode.ADD and not cloned_part.has_polygons
                ):
