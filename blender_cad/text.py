...
from .common import PartLike, extract_part
from .curve import BaseCurve, Curve
from .location import Location
from .material import mat
from .modifiers import WrapMode
...
class t:
    ...
    def plain(self) -> str:
        ...
        return "".join(
            child if isinstance(child, str) else child.plain for child in self.children
        )
    ...
    def flatten(self, parent_style: Optional[dict] = None) -> list[tuple[str, dict]]:
        ...
        if self.mat is not None:
            current_style["mat"] = self.mat
        if self.bold is not None:
            current_style["bold"] = self.bold
        if self.italic is not None:
            current_style["italic"] = self.italic
...
class Text(BaseCurve):
    ...
    def __init__(
        ...
        self.align("LEFT", "BOTTOM_BASELINE")
    ...
    def max_width(self) -> float:
        return self.obj.data.text_boxes[0].width

    @max_width.setter
    def max_width(self, value: float):
        self.obj.data.text_boxes[0].width = value
    ...
    @property
    def spacing_word(self) -> float:
        return self.obj.data.space_word

    @spacing_word.setter
    def spacing_word(self, value: float):
        self.obj.data.space_word = value

    @property
    def spacing_line(self) -> float:
        return self.obj.data.space_line

    @spacing_line.setter
    def spacing_line(self, value: float):
        self.obj.data.space_line = value

    @property
    def offset(self) -> float:
        return self.obj.data.offset

    @offset.setter
    def offset(self, value: float):
        self.obj.data.offset = value
    ...
    def load_fonts(
        ...
        if bold:
            self.obj.data.font_bold = get_font(bold)
        if italic:
            self.obj.data.font_italic = get_font(italic)
        if bold_italic:
            self.obj.data.font_bold_italic = get_font(bold_italic)
        return self
    ...
    def put_on_curve(self, curve: Curve) -> "Text":
        ...
        self.obj.modifiers.clear()
        mod: bpy.types.CurveModifier = self.obj.modifiers.new(
            name="CurvePath", type="CURVE"
        )
    ...
    def wrap(
        ...
        *,
        ...
        """
        Shrinkwraps text onto a surface.
        """
        ...
        self.obj.modifiers.clear()
        mod: bpy.types.ShrinkwrapModifier = self.obj.modifiers.new(
            name="TextWrap", type="SHRINKWRAP"
        )

        target_part = extract_part(target)
        mod.target = target_part.obj
        mod.wrap_method = mode.value
        mod.offset = offset

        # Store dependency so .part knows what to bring onto the scene
        self._dependencies = [target_part]
        return self
