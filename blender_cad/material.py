...
class MaterialValue(ABC):
    ...
    @abstractmethod
    def _build_impl(
        self, ctx: "BuildMaterial"
    ) -> bpy.types.NodeSocket | float | Vector:
        raise NotImplementedError
    ...
    def __truediv__(self, other: object):
        return BinaryExpr(self, MathOp.DIVIDE, other)
...
class BinaryExpr(MaterialValue):
    ...
    def stable_key(self) -> tuple[object, ...]:
        return (
            ...
            self.kind.value,
            self.op,
            _stable_any(self.left),
            _stable_any(self.right),
        )
    ...
    def _build_impl(
        ...
        if _is_socket(left_val) or _is_socket(right_val):
            node: bpy.types.ShaderNodeMath = ctx.new_node(
                NodeType.MATH, label=f"Expr {self.op.value}"
            )
...
class VariableExpr(MaterialValue):
    ...
    def _build_impl(
        ...
        if self.kind == ValueKind.FLOAT:
            node: bpy.types.ShaderNodeValue = ctx.new_node(
                NodeType.VALUE, label=f"Var {self.name}"
            )
            node.outputs[0].default_value = _as_float(self.default)
            result = node.outputs[0]
        elif self.kind == ValueKind.VECTOR:
            node: bpy.types.ShaderNodeCombineXYZ = ctx.new_node(
                NodeType.COMBINE_XYZ, label=f"Var {self.name}"
            )
            ...
            node: bpy.types.ShaderNodeRGB = ctx.new_node(
                NodeType.RGB, label=f"Var {self.name}"
            )
...
class MixExpr(MaterialValue):
    ...
    def _build_impl(self, ctx: "BuildMaterial") -> bpy.types.NodeSocket:
        ...
        mix: bpy.types.ShaderNodeMix = ctx.new_node(
            NodeType.MIX, label=f"Mix {self.mode.name}"
        )
...
class MappingExpr(MaterialValue):
    ...
    def _build_impl(self, ctx: "BuildMaterial") -> bpy.types.NodeSocket:
        ...
        ctx._apply_to_socket(
            mapping_node.inputs["Location"], tr.position, kind=ValueKind.VECTOR
        )
        ctx._apply_to_socket(
            mapping_node.inputs["Rotation"], tr.euler_rad, kind=ValueKind.VECTOR
        )
        ctx._apply_to_socket(
            mapping_node.inputs["Scale"], tr.scale, kind=ValueKind.VECTOR
        )
...
class CameraTexture(BaseTexture):
    ...
    _cached_image: bpy.types.Image | None = field(
    ...
    def build_image(self) -> bpy.types.Image:
        ...
        image = _render_camera_to_image(
            self.location, self.fov, self.resolution, self.get_label()
        )
...
class TextureExpr(MaterialValue):
    ...
    def _build_impl(self, ctx: "BuildMaterial") -> bpy.types.NodeSocket:
        ...
        if img_node is None:
            img_node: bpy.types.ShaderNodeTexImage = ctx.new_node(
                NodeType.TEX_IMAGE, label=self.texture.get_label()
            )
...
class NoiseExpr(MaterialValue):
    ...
    def stable_key(self) -> tuple[object, ...]:
        return (
            "noise",
            _stable_any(self.scale),
            _stable_any(self.detail),
            _stable_any(self.roughness),
            _stable_any(self.distortion),
            _stable_any(self.mapping),
            self.kind.value,
        )
...
class BrickExpr(MaterialValue):
    ...
    def stable_key(self) -> tuple[object, ...]:
        return (
            "brick",
            _stable_any(self.color1),
            _stable_any(self.color2),
            _stable_any(self.mortar),
            _stable_any(self.scale),
            _stable_any(self.mortar_size),
            _stable_any(self.mapping),
            self.kind.value,
        )
    ...
    def _build_impl(self, ctx: "BuildMaterial") -> bpy.types.NodeSocket:
        ...
        return (
            node.outputs["Color"]
            if self.kind == ValueKind.RGBA
            else node.outputs["Fac"]
        )
...
class ColorRampExpr(MaterialValue):
    ...
    stops: tuple[tuple[float, tuple[float, float, float, float]], ...] = (
        (0.0, (0, 0, 0, 1)),
        (1.0, (1, 1, 1, 1)),
    )
...
class BumpExpr(MaterialValue):
    ...
    def stable_key(self) -> tuple[object, ...]:
        return (
            "bump",
            _stable_any(self.height),
            _stable_any(self.strength),
            _stable_any(self.distance),
            self.kind.value,
        )
    ...
    def stable_key(self) -> tuple[object, ...]:
        if is_dataclass(self):
            return (self.__class__.__name__,) + tuple(
                _stable_any(getattr(self, f.name)) for f in fields(self)
            )
        return (self.__class__.__name__,) + tuple(
...
class MaterialCompositeLayer(MaterialLayer):
    ...
    @classmethod
    def from_terms(
        cls, terms: Sequence[MaterialWeightedLayer], op: str = "add"
    ) -> "MaterialCompositeLayer":
    ...
    def __add__(self, other: MaterialExpr) -> "MaterialCompositeLayer":
        return MaterialCompositeLayer.from_terms(
            self.terms + (_as_weighted(other, 1.0),)
        )
    ...
    def __mul__(self, factor: object) -> "MaterialCompositeLayer":
        return MaterialCompositeLayer(
            tuple(
                MaterialWeightedLayer(
                    MaterialWeightedLayer(term.layer, term.factor), factor
                )
...
class MaterialMeta:
    ...
    def as_key(self) -> tuple[object, ...]:
        custom_items = tuple(
            sorted((k, _stable_any(v)) for k, v in self.custom.items())
        )
...
class BuildMaterial:
    ...
    def __init__(self, material: bpy.types.Material) -> None:
        ...
        self._expr_cache: dict[
            tuple[object, ...], bpy.types.Node, bpy.types.NodeSocket | float | Vector
        ] = {}
    ...
    def factor(self) -> object:
        ...
        for val in self._factor_stack:
            ...
            if (
                ...
                or hasattr(val, "__mul__")
                or hasattr(val, "__rmul__")
            ):
                folded_groups[val_type] = (
                    folded_groups[val_type] * val if val_type in folded_groups else val
                )
            else:
        ...
        for folded_val in folded_groups.values():
            ...
            if result_val is None:
                ...
                result_val = BinaryExpr(
                    left=result_val, op=MathOp.MULTIPLY, right=folded_val
                )
    ...
    def resolve(
        self, value: object, kind: ValueKind = ValueKind.FLOAT
    ) -> bpy.types.NodeSocket | float | Vector:
    ...
    def finish(self) -> bpy.types.Material:
        ...
        output: bpy.types.ShaderNodeOutputMaterial = self.new_node(
            NodeType.OUTPUT_MATERIAL, x=300, y=0
        )
        bsdf: bpy.types.ShaderNodeBsdfPrincipled = self.new_node(
            NodeType.BSDF, x=0, y=0
        )
        ...
        for attr, spec in CHANNEL_SPECS.items():
            ...
            if attr == "normal":
                ...
                if _is_socket(resolved) and resolved.type == "VECTOR":
                    ...
                    nmap_node: bpy.types.ShaderNodeNormalMap = self.new_node(
                        NodeType.NORMAL_MAP
                    )
                    self._apply_to_socket(
                        nmap_node.inputs["Color"], resolved, spec.kind
                    )
...
def _infer_kind(a: object, b: object) -> ValueKind:
    ...
    priority = {ValueKind.FLOAT: 0, ValueKind.VECTOR: 1, ValueKind.RGBA: 2}

    return kind_a if priority[kind_a] >= priority[kind_b] else kind_b


def _python_blend(
    a: object, b: object, factor: float, mode: BlendMode, kind: ValueKind
):
    """
    ...
    def to_vec4(val: object) -> Vector:
        ...
        if isinstance(val, (Vector, Color, tuple, list)):
            l = len(val)
            if l == 1:
                return Vector((val[0], val[0], val[0], 1.0))
            if l == 3:
                return Vector((val[0], val[1], val[2], 1.0))
            if l >= 4:
                return Vector((val[0], val[1], val[2], val[3]))
    ...
    if mode == BlendMode.MIX:
        ...
        raise NotImplementedError(
            f"Blend mode {mode} not supported for constant folding."
        )
...
def _build_or_get_image(image_ref: str) -> bpy.types.Image:
    ...
    if img is not None and bpy.path.abspath(getattr(img, "filepath", "")).endswith(
        filename
    ):
        return img
...
def _render_camera_to_image(
    location: Location, fov: float, resolution: tuple[int, int], label="camera_render"
) -> bpy.types.Image:
    ...
    def hash_node(node: bpy.types.Node) -> str:
        ...
        for prop in node.bl_rna.properties:
            ...
            if prop.identifier in ignored_props or prop.identifier.startswith("bl_"):
                continue
            data.append(
                f"p:{prop.identifier}:{format_value(getattr(node, prop.identifier))}"
            )
    ...
    def build(self, ctx: BuildMaterial) -> None:
        ...
        for attr in attrs:
            ...
            setattr(
                ctx.channels,
                attr,
                ctx.blend(current, incoming, mode=self.mode)
...
class Metal(MaterialLayer):
    ...
    def build(self, ctx: BuildMaterial) -> None:
        ...
        roughness_map = ctx.map_range(
            noise,
            from_min=0.3,
            from_max=0.7,
            to_min=self.roughness_min,
            to_max=self.roughness_max,
        )
...
class Concrete(MaterialLayer):
    ...
    def build(self, ctx: BuildMaterial) -> None:
        ...
        pits = ctx.map_range(
            voronoi, from_min=0.0, from_max=0.3, to_min=1.0, to_max=0.0
        )
        surface = ctx.blend(noise, pits, factor=0.5, mode=BlendMode.SUBTRACT)

        ctx.channels.base_color = ctx.blend(
            self.color, Vector((0.2, 0.2, 0.2, 1.0)), factor=pits
        )
        ctx.channels.metallic = 0.0
...
class Brick(MaterialLayer):
    ...
    @override
    def build(self, ctx: BuildMaterial) -> None:
        brick_color = ctx.brick(
            color1=self.color1,
            color2=self.color2,
            scale=self.scale,
            kind=ValueKind.RGBA,
        )
        brick_mask = ctx.brick(
            scale=self.scale, kind=ValueKind.FLOAT
        )  # Returns mortar mask via fac
...
class Fabric(MaterialLayer):
    ...
    color: object = field(
        default_factory=lambda: Vector((0.1, 0.2, 0.6, 1.0))
    )  # Denim default
    scale: float = 50.0
    ...
    def build(self, ctx: BuildMaterial) -> None:
        ...
        ctx.channels.base_color = ctx.blend(
            self.color, Vector((0.0, 0.0, 0.0, 1.0)), factor=weave * 0.2
        )
        ctx.channels.metallic = 0.0
...
class Leather(MaterialLayer):
    ...
    def build(self, ctx: BuildMaterial) -> None:
        ...
        ctx.channels.base_color = ctx.blend(
            self.color, Vector((0.15, 0.05, 0.02, 1.0)), factor=pebbles
        )
        ctx.channels.metallic = 0.0
...
class Glow(MaterialLayer):
    ...
    def build(self, ctx: BuildMaterial) -> None:
        ctx.channels.emission_color = ctx.blend(
            ctx.channels.emission_color, self.emission_color
        )
        ctx.channels.emission_strength = ctx.blend(
            ctx.channels.emission_strength, self.emission_strength
        )
...
class Dirt(MaterialLayer):
    ...
    def build(self, ctx: BuildMaterial) -> None:
        ...
        ctx.channels.base_color = ctx.blend(
            ctx.channels.base_color, self.color, factor=combined_mask
        )
        ctx.channels.roughness = ctx.blend(
            ctx.channels.roughness, 0.9, factor=combined_mask
        )
...
class DirtImg(MaterialLayer):
    ...
    texture: Texture = field(
        default_factory=lambda: Texture(image_path="./assets/test/dirt.png")
    )
...
class Rust(MaterialLayer):
    ...
    def build(self, ctx: BuildMaterial) -> None:
        ...
        ctx.channels.base_color = ctx.blend(
            ctx.channels.base_color, rust_colors, factor=final_factor
        )
        ctx.channels.metallic = ctx.blend(
            ctx.channels.metallic, 0.0, factor=final_factor
        )
        ctx.channels.roughness = ctx.blend(
            ctx.channels.roughness, 0.95, factor=final_factor
        )
...
class RustImg(MaterialLayer):
    ...
    texture: Texture = field(
        default_factory=lambda: Texture(image_path="./assets/test/rust.png")
    )
...
class Dust(MaterialLayer):
    ...
    def build(self, ctx: BuildMaterial) -> None:
        ...
        ctx.channels.base_color = ctx.blend(
            ctx.channels.base_color, self.color, factor=final_factor
        )
        ...
        ctx.channels.roughness = ctx.blend(
            ctx.channels.roughness, self.roughness, factor=final_factor
        )
        # Dust blocks transmission (transparency)
        if ctx.channels.transmission is not None:
            ctx.channels.transmission = ctx.blend(
                ctx.channels.transmission, 0.0, factor=final_factor
            )
...
class Cracks(MaterialLayer):
    ...
    def build(self, ctx: BuildMaterial) -> None:
        ...
        crack_mask = ctx.map_range(
            voronoi_base, from_min=0.0, from_max=self.thickness, to_min=1.0, to_max=0.0
        )
        ...
        ctx.channels.base_color = ctx.blend(
            ctx.channels.base_color, self.dirt_color, factor=final_mask
        )

        # Roughness: Cracks are usually very rough (dusty/dry)
        ctx.channels.roughness = ctx.blend(
            ctx.channels.roughness, 1.0, factor=final_mask
        )
        ...
        ctx.channels.normal = ctx.blend(
            ctx.channels.normal,
            ctx.bump(height=height_map, strength=self.bump_strength),
            factor=final_mask,
        )
...
class mat:
    ...
    iron = Metal(
        color=Vector((0.3, 0.3, 0.3, 1.0)), roughness_min=0.2, roughness_max=0.5
    ) + Name("IronMat")
    gold = Metal(
        color=Vector((1.0, 0.75, 0.1, 1.0)), roughness_min=0.1, roughness_max=0.2
    ) + Name("GoldMat")
    copper = Metal(
        color=Vector((0.8, 0.3, 0.2, 1.0)), roughness_min=0.15, roughness_max=0.4
    ) + Name("CopperMat")

    wood_oak = Wood(
        color_dark=Vector((0.25, 0.12, 0.04, 1.0)),
        color_light=Vector((0.55, 0.35, 0.15, 1.0)),
    ) + Name("WoodOakMat")
    wood_pine = Wood(
        color_dark=Vector((0.6, 0.4, 0.2, 1.0)),
        color_light=Vector((0.8, 0.6, 0.3, 1.0)),
        scale=2.0,
    ) + Name("WoodPineMat")
    ...
    plastic_glossy_red = Plastic(
        color=Vector((0.8, 0.05, 0.05, 1.0)), roughness=0.1
    ) + Name("PlasticGlossyRedMat")
    plastic_matte_grey = Plastic(
        color=Vector((0.3, 0.3, 0.3, 1.0)), roughness=0.6
    ) + Name("PlasticMatteGreyMat")
