...
class TestSelectorsAndLocations(BaseCADTest):
    ...
    def test_geometry_type_identification(self):
        ...
        self.assertEqual(
            len(spherical_faces), 1, "Sphere must have at least 1 spherical face"
        )
    ...
    def test_at_with_offset(self):
        ...
        self.assertPart(
            result.part,
            "e041c96775995207b21c21b804230cdca7103a4338531a4ed8cd8133fe35dcf2",
            "test_at_with_offset",
        )
    ...
    def test_box_selectors_and_grids(self):
        ...
        with BuildPart() as result:
            ...
            self.iter(3, lambda u: self.iter(3, lambda v: Marker(top_face.at(u, v))))

            # 3. Mark points along the longest edges
            long_edges = (
                box_faces.sort_by(Axis.Z)[0].edges().sort_by(SortBy.LENGTH)[-2:]
            )
        ...
        self.assertPart(
            result.part,
            "7ff1913b1c27e0789da4da9c83bf9bb7905e441d92a430c157486e72f269544c",
            "test_box_selectors_and_grids",
        )
    ...
    def test_sphere_surface_mapping(self):
        ...
        self.assertPart(
            result.part,
            "fa556fcd13e93371531ec74ee4be8a0deb5a6b340da96bf4ab106463c9c44880",
            "test_sphere_surface_mapping",
        )
    ...
    def test_cone_rectangularization(self):
        ...
        self.assertPart(
            result.part,
            "e3e8120bb9a9042de836d50b8ae101650f9ec044d9ab4bb2918c0c03447b5b2d",
            "test_cone_rectangularization",
        )
    ...
    def test_rotated_cylinder_wire(self):
        ...
        self.assertPart(
            result.part,
            "6bf49b4670524c94c9fec7d4446949d266390ffc898233d8fb5d0a11eda19ff7",
            "test_rotated_cylinder_wire",
        )
    ...
    def test_rotated_annulus_wire(self):
        ...
        self.assertPart(
            result.part,
            "52e81aa80f66ed42db7d1895152ad3b943d2f2f1d1f79dcee6e4bab66df679e8",
            "test_rotated_annulus_wire",
        )

    def test_wire_creation_from_filtered_edges_and_curve_distribution(self):
        """Verify that to_wire successfully builds a continuous path from subset edges to distribute markers."""
        with BuildPart() as result:
            # Step 1: Create a basic solid box reference geometry
            Box(2, 2, 2)

            # Step 2: Filter out the maximum Y edge from the top edges and convert the remainder into a clean 3D wire
            wire = (edges().top() - edges().max_y()).to_wire()

            # Step 3: Distribute structural Marker entities evenly at 8 distinct points along the constructed wire path
            with CurveLocations(wire, count=8):
                Marker()

        self.assertPart(
            result.part,
            "8c7f8f9bef385871c141014128d6794e6a0cce52946d4319a0db4c40448b71ee",
            "test_wire_creation_from_filtered_edges_and_curve_distribution",
        )
    ...
    def test_internal_cylinder_surface(self):
        ...
        self.assertPart(
            result.part,
            "820f59cd9e5fbaecf31cba132fedeb88711a734c9181e765f9625689cb83940d",
            "test_internal_cylinder_surface",
        )
    ...
    def test_grouping_and_filtering(self):
        ...
        self.assertPart(
            result.part,
            "da49faa0ed9bc48d221d46a46edf38d42c36bdcf9e3961f6c319007a30a1bd8c",
            "test_grouping_and_filtering",
        )
    ...
    def test_complex_selection_logic(self):
        ...
        with BuildPart() as result:
            ...
            z_groups = faces().group_by(Axis.Z)
            selected_faces = (z_groups[-1] & x_groups[-3]) + (
                z_groups[-2] - ~(x_groups[-2] + x_groups[1])
            )
        ...
        self.assertPart(
            result.part,
            "6b59c6b0b942029004bc74d014d7ab4fe305f8eb25588b622acb9c0c391f468c",
            "test_complex_selection_logic",
        )
    ...
    def test_sphere_cone_attachment_and_selection(self):
        ...
        self.assertPart(
            result.part,
            "17c0177f117709f1f53c3e4fa28a65ef0f5988afa3897444ca3770e9d2835b9f",
            "test_sphere_cone_attachment_and_selection",
        )
    ...
    def test_surface_at_mapping_with_complex_deformation(self):
        ...
        with BuildPart() as result:
            ...
            result.transform *= (
                Pos(1, 2, 3) * Rot(X=30, Y=40, Z=50) * FlipZ * Scale(1, 2, 3)
            )
    ...
    def test_stretched_sphere_surface_with_grid(self):
        ...
        with BuildPart() as result:
            ...
            with Locations(sphere_face.location(uv.offset(v=0.2))):
    ...
    def test_stretched_box_surface_with_grid(self):
        ...
        with BuildPart() as result:
            ...
            with Locations(top_face.location(uv)):
    ...
    def test_half_cylinder_surface_mapping(self):
        ...
        with BuildPart() as result:
            ...
            with Locations(cyl_face.location(uv)):
                with GridLocations(1, 1, 2, 2):
                    Marker(size=0.5)
    ...
    def test_polygon_free_curve_mesh(self):
        """Verify that the topological analyzer correctly processes a raw 3D mesh consisting only of edges and vertices without faces."""
        with BuildPart(topology=TopologyConfig(edge_break_angle=15)) as result:
            # Step 1: Create a closed 2D spline path within a curve context to generate pure edge topology
            with BuildCurve() as bc:
                Spline((0, 0, 0), (10, 0, 0), (10, 10, 0), close=True)

            # Step 2: Inject the polygon-free wire into the active part environment
            add(bc)

            # Step 3: Extract topological collections for validation and discrete targeting
            all_edges = edges()
            all_wires = wires()

            # Step 4: Distribute red markers symmetrically along the first comprehensive wire path loop
            with CurveLocations(all_wires[0], count=10):
                Marker(loc=Pos(Z=0), mat=mat.red)

            # Step 5: Isolate individual discrete edges and distribute uniquely color-coded tracking markers along them
            # Target the first sub-edge with green indicators
            with CurveLocations(all_edges[0], count=5):
                Marker(loc=Pos(Z=1), mat=mat.green)

            # Target the second sub-edge with blue indicators
            with CurveLocations(all_edges[1], count=5):
                Marker(loc=Pos(Z=1), mat=mat.blue)

            # Target the third sub-edge with yellow indicators
            with CurveLocations(all_edges[2], count=5):
                Marker(loc=Pos(Z=1), mat=mat.yellow)

        self.assertPart(
            result.part,
            "0629a8f454a43acd05e78aadf322e2c8a9aa89314d6fbe8be623d5572d5c84f3",
            "test_polygon_free_curve_mesh",
            use_materials=True,
        )
...
class TestGeometryCheckpoints(BaseCADTest):
    def test_stacked_boxes_no_overlap(self):
        ...
        self.assertPart(
            result.part,
            "8bd570ed24140f2a9202af98d0cfa66025b10177fda629460f82099af2d80bee",
            "test_stacked_boxes_no_overlap",
            use_materials=True,
        )
    ...
    def test_overlapping_boxes_union(self):
        ...
        self.assertPart(
            result.part,
            "d4720a970bc079a3620067bee14debcb776e2ad3d0eec85c227b0a1dc127383e",
            "test_overlapping_boxes_union",
            use_materials=True,
        )
    ...
    def test_boolean_subtraction_cutout(self):
        ...
        self.assertPart(
            result.part,
            "5d6d9c24dbd8df32dbb1fd6cf4a7530cdf197e7bbf8c8b0c6117a20406d9eb28",
            "test_boolean_subtraction_cutout",
            use_materials=True,
        )
    ...
    def test_plane_subtraction(self):
        ...
        self.assertPart(
            result.part,
            "07886bc191e2685b2001ddc0a9f0ac97eb0e2022312c1817b6ce82a386900e06",
    ...
    def test_separate_planes(self):
        ...
        self.assertPart(
            result.part,
            "f23fbd7b177866ffae3367572496adc0b499cde217271598774a530361c72b8f",
            "test_separate_planes",
            use_materials=True,
        )
    ...
    def test_extrude_wire_selection(self):
        ...
        self.assertPart(
            result.part,
            "923f11f9f4e28d1317f73fcc03b47f8eb17cc66765b4af1cae7dad82a35ea1f8",
            "test_extrude_wire_selection",
            use_materials=True,
        )
    ...
    def test_manual_checkpoint_persistence(self):
        ...
        self.assertPart(
            result.part,
            "c6ad3496eee4c969f54c223ceeaac345b87d1becf4b2fa8af4b1bc004b947935",
            "test_manual_checkpoint_persistence",
            use_materials=True,
        )
    ...
    def test_subdivision_topology_change(self):
        ...
        self.assertPart(
            result.part,
            "2d0d6650aa9eec3ea1bb73d38155b21def230e20e1b82a78dd6a608bf1281952",
            "test_subdivision_topology_change",
            use_materials=True,
        )
    ...
    def test_bevel_selection(self):
        ...
        self.assertPart(
            result.part,
            "08654681fe8ee37193b6b304953833aec60d36dac5cdb2d6f6292c42f6fea35e",
            "test_bevel_selection",
            use_materials=True,
        )


class TestTags(BaseCADTest):
    def test_geometry_tag_system_propagation_and_selection(self):
        """Verify that custom tags can be assigned to faces, propagate through operations, and remain selectable."""
        with BuildPart() as result:
            # Step 1: Create parent geometry and tag the top face
            Box(1, 2, 0.1)
            faces().top().add_tags("parent_top")

            # Step 2: Apply a deformation (tags should survive the bend operation)
            bend(angle=90, axis=Axis.X, segments=10)

            # Step 3: Create and tag nested child geometry inside a local context
            with BuildPart(offset=Pos(Z=0.1)):
                Box(0.1, 0.1, 0.1)
                faces().side().add_tags("child_side")

            # Step 4: Transform the combined part and sub-select a specific tagged area
            transform(op=Rot(X=90))
            faces().tagged("parent_top").split().min_y().add_tags("parent_min_y")

            # Step 5: Assert that all three tags exist within the active face collection
            self.assertIn("parent_top", faces().tags)
            self.assertIn("child_side", faces().tags)
            self.assertIn("parent_min_y", faces().tags)

            # Step 6: Apply materials targeting specific tagged elements to verify selection works
            faces().tagged("parent_top").mat = mat.red
            faces().split().tagged("child_side").mat = mat.blue
            faces().split().tagged("parent_min_y").mat = mat.green

        self.assertPart(
            result.part,
            "51d9aefd10d011b9f40b7a98de146482237e186324bf642d85ca1ee08f7e57e9",
            "test_geometry_tag_system_propagation_and_selection",
            use_materials=True,
        )

    def test_extrude_operation_automatic_tagging_and_selection(self):
        """Verify that any new geometry generated during extrude operations is automatically tagged and selectable."""
        with BuildPart() as result:
            # Step 1: Create a base solid reference body
            Box(1, 1, 1)

            # Step 2: Perform consecutive extrude operations on vertices, faces, and edges
            extrude(vertices().max_x(), op=Pos(X=1))
            extrude(faces().top(), op=Pos(Z=1))
            extrude(edges().bottom(), op=Pos(Z=-1))

            # Step 3: Isolate and extract all newly generated faces, edges, and vertices using the operation tag
            new_faces = faces().split().tagged(Part.TAG_OP_EXTRUDE)
            new_edges = edges().split().tagged(Part.TAG_OP_EXTRUDE)
            new_vertices = vertices().split().tagged(Part.TAG_OP_EXTRUDE)

            # Step 4: Iterate over the newly generated elements to place color-coded structural indicators
            for f in new_faces:
                Marker(f.center(), mat=mat.red)
            for e in new_edges:
                Marker(e.center(), mat=mat.green)
            for v in new_vertices:
                Marker(v.center(), mat=mat.blue)

        self.assertPart(
            result.part,
            "e7da644696cc74ecded622d9d10f23fb76318640a287d25085a2791c4ae29bd0",
            "test_extrude_operation_automatic_tagging_and_selection",
            use_materials=True,
        )

    def test_markup_layout_automatic_border_and_background_tagging(self):
        """Verify that ml components automatically tag border and background elements, allowing independent downstream manipulation."""
        with BuildPart() as result:
            # Step 1: Build a markup layout with explicit border styles and background properties
            ml(
                style(
                    width=4,
                    height=4,
            ...
            # Step 2: Target only the layout border edges using the automatic layout tag and extrude them
            extrude(edges().tagged(Part.TAG_ML_BORDER), op=Pos(Z=-1))

            # Step 3: Isolate the layout background faces via their systemic tag and scale them down
            transform(
                faces().tagged(Part.TAG_ML_BACKGROUND), op=Origin(0.5) * Scale(XY=0.5)
            )

        self.assertPart(
            result.part,
            "201098738fd2819d4862bb3d393b5e4e128d630546955409d9706d0af1e56108",
            "test_markup_layout_automatic_border_and_background_tagging",
            use_materials=True,
        )
