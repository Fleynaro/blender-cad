...
class BaseCADTest(unittest.TestCase):
    ...
    def assertPart(self, result_part: Part, expected_hash: str | Iterable[str], test_name: str, use_materials=False, precision=4):
        """
        ...
        allowed_hashes = [expected_hash] if isinstance(expected_hash, str) else list(expected_hash)
        actual_hash = result_part.hash(use_materials=use_materials, precision=precision)
        try:
            if actual_hash not in allowed_hashes:
                raise AssertionError(
                    f"Hash mismatch in {test_name}! Expected one of {', '.join(allowed_hashes)}, got {actual_hash}"
                )
