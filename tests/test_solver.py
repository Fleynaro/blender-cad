...
class TestSolverOptimization(BaseCADTest):
    ...
    def test_single_parameter_optimization(self):
        ...
        self.assertAlmostEqual(
            ...
            5.0,
            places=4,
            msg="Solver failed to find single parameter root.",
        )
    ...
    def test_single_integer_parameter_optimization(self):
        ...
        self.assertEqual(
            final_y, 5, f"Solver should have snapped to 5, but got {final_y}"
        )
    ...
    def test_solver_caching(self):
        ...
        self.assertGreater(
            first_run_iters,
            second_run_iters,
            "Cache failed: First run took more iterations than second run.",
        )
        self.assertLessEqual(
            second_run_iters,
            4,
            f"Cache failed: Second run took {second_run_iters} iterations.",
        )
    ...
    def test_brute_mixed_int_float(self):
        ...
        self.assertEqual(final_x, 4, "Failed: X should be 4")
        self.assertAlmostEqual(final_y, 7.5, places=3, msg="Failed: Y should be 7.5")
    ...
    def test_matrix_as_nested_list(self):
        ...
        initial_matrix = [[(0.0, (None, 2.0)), 0.0], [0.0, 0.0]]
    ...
    def test_pos_array_with_bounds(self):
        ...
        points_spec = [(points_init[0], (Pos(0, 0, 0), Pos(0, 0, 0))), points_init[1]]
        ...
        self.assertAlmostEqual(
            (res_points[0].position - res_points[1].position).length, 10.0, places=2
        )
    ...
    def test_dynamic_param_calls_in_loop(self):
        ...
        for s in Solver(max_steps=500, tol=1e-6):
            ...
            for i in range(len(initial_values)):
                ...
                val = s.param(initial_values[i], min=0.0, max=max_bounds[i])
    ...
    def test_homotopy_constraint_relaxation_with_tracking(self):
        """
        Test optimizing a parameter under a relaxed constraint using the repeat() method.

        Verifies that during the early relaxed stages (s.homotopy_parameter <= 0.5),
        X safely explores up to 10.0, but snaps back to 6.0 on the final step.
        """
        final_x = 0.0
        max_x = 0.0

        # Execute optimization steps via the new chain setup
        for s in sm.nelder_mead(tol=1e-4).repeat(2):
            # Define the parameter to optimize
            X = s.param(0.0, min=0.0, max=15.0)

            # Track the maximum exploratory value encountered across all evaluations
            max_x = max(max_x, X)

            # 1. Main objective: We want X to reach 10.0
            s.aim_equal(X, 10.0)

            # 2. Homotopy-driven constraint: Keep X below 6.0
            # Early steps (t <= 0.5) ignore this rule completely, allowing max exploration.
            # Later steps enforce a massive quadratic penalty.
            if X > 6.0 and s.homotopy_parameter > 0.5:
                s.aim(1000000.0 * (X - 6.0) ** 2)

            # Capture the result at the final execution step
            if s.is_final:
                final_x = X

        # 1. Check that X was able to freely reach or exceed 10 during relaxation
        self.assertGreaterEqual(
            max_x, 10.0, f"Exploration failed: max_x was only {max_x}, expected >= 10.0"
        )

        # 2. Check that the final constrained state successfully pulled X down to 6.0
        self.assertAlmostEqual(
            final_x,
            6.0,
            places=2,
            msg=f"Homotopy failed to enforce the final boundary. final_x got {final_x}",
        )
