...
class TestLocationsAndPatterns(BaseCADTest):
    ...
    def test_surface_coordinate_chaining_and_compensation(self):
        ...
        with BuildPart() as result:
            ...
            target_loc: SurfaceLocation = (
                Pos(Z=1) * Pos(Z=1) * cyl_face.location(uv) * Pos(X=1, Z=1) * Pos(X=1, Z=-1) * Pos(Y=2, Z=0.2) * Rot(Y=90)
            )
    ...
    def test_bbox_joint_alignment_across_multiple_axes(self):
        ...
        with BuildPart() as result:
            ...
            box.part.bbox_joint(Axis.X).to(result.part.bbox_joint(-Axis.X), twist=0, mode=Mode.JOIN, mat=mat.green)
            
            # Connect along the positive X axes
            box.part.bbox_joint(Axis.X).to(result.part.bbox_joint(Axis.X), twist=0, mode=Mode.JOIN, mat=mat.blue)
            
            # Connect along the Y and negative Y axes
            box.part.bbox_joint(Axis.Y).to(result.part.bbox_joint(-Axis.Y), twist=0, mode=Mode.JOIN, mat=mat.yellow)
    ...
    def test_joint_registration_and_propagation(self):
        ...
        with BuildPart() as result:
            ...
            result.part.register_joint("top", result.part.joint(faces().top()[0].at(uv)))
            ...
            with BuildPart(mode=Mode.PRIVATE) as child:
                ...
                child.part.register_joint("child_top", child.part.joint(faces().top()[0].at(uv)), propagate=True)
                # Register a bottom joint on the child without propagation
                child.part.register_joint("child_bottom", child.part.joint(faces().bottom()[0].at(uv)))

            # Connect child's bottom joint to the base part's top joint (triggers propagation for child_top)
            child.part.joint_by_name("child_bottom").to(result.part.joint_by_name("top"))
            # Verify the propagated child_top joint can be used for a subsequent connection
            child.part.joint_by_name("child_top").to(result.part.joint_by_name("child_top"))

        # Assert the final combined geometry hash to ensure correct transformation and assembly
        self.assertPart(result.part, "22816509e7e66002b63fe8c6c5f0fdbcbdab937bc9e23f5310003f5994c3716a", "test_joint_registration_and_propagation")
