...
class TestLocationsAndPatterns(BaseCADTest):
    ...
    def test_independent_loc_and_scale(self):
        ...
        self.assertPart(
            result.part,
            "faa5220b1bb0d325a9ba1af6030441f6b5fe263cdb5f7fe4be16bcf6caf4bd9c",
            "test_independent_loc_and_scale",
        )
    ...
    def test_grid_and_rotational_locations(self):
        ...
        self.assertPart(
            result.part,
            "6c61662c324de2a317d069f31043a6e5290cde8896e0314f222c5424c480c3cb",
            "test_grid_and_rotational_locations",
        )
    ...
    def test_polar_distribution(self):
        ...
        self.assertPart(
            result.part,
            "6c270f31de94289dcada98cbb334326ae9e2a88a94a20aace75a66e372539d1a",
            "test_polar_distribution",
        )
    ...
    def test_hexagonal_distribution(self):
        ...
        self.assertPart(
            result.part,
            "9bd253f01cc649637642bf3325a0d19f1a68a985a2e41405e9f729ff15375ac9",
            "test_hexagonal_distribution",
        )
    ...
    def test_surface_to_global_transition_via_loc(self):
        ...
        with BuildPart() as result:
            ...
            surf_loc: SurfaceLocation = cone_face.location(uv) * Pos(X=0.5, Z=0.1)
    ...
    def test_surface_with_grid_and_cumulative_rotation(self):
        ...
        with BuildPart() as result:
            ...
            with Locations(cyl_face.location(uv.set(v=0.5))):
    ...
    def test_cylinder_with_wire_location_and_cumulative_rotation(self):
        """
        Verify that a wire location can be extracted and used as a base for nested
        transformations, and that nested rotations (10 + 20 = 30 degrees)
        are accumulated correctly on the placed Marker.
        """
        # Setup: A cylinder with radius 1, height 2
        with BuildPart() as result:
            Cylinder(1, 2)

            # 1. Start from the location of the first top wire
            with Locations(wires().top()[0].location()):
                # 2. Apply a local X and Z offset along with a partial rotation
                with Locations(Pos(X=2, Z=0.2) * Rot(X=10)):
                    # 3. Apply the remaining rotation and place a marker
                    # Total Rotation should be Rot(X=30)
                    Marker(Rot(X=20))
        ...
        # This checks that:
        # - 1 Marker exists.
        # - Marker is correctly positioned relative to the top wire.
        # - Marker has a combined X-rotation of 30 degrees.
        self.assertPart(
            result.part,
            "584f10f79e748282ef521f62c4d115b51c7b67830cd80598b66bc916ea123dc2",
            "test_cylinder_with_wire_location_and_cumulative_rotation",
        )
    ...
    def test_align_joints(self):
        ...
        self.assertPart(
            result.part,
            "6b067a1072e5af18e25e3fedbbf49960969c2679254c60b3a9c615de4965f67e",
            "test_align_joints",
        )
    ...
    def test_align_joints_with_twist(self):
        ...
        self.assertPart(
            result.part,
            "40511e9e4740266a7e85c80032ec7d3ff41ed194ac74272ca1b4852477910cab",
            "test_align_joints_with_twist",
        )
    ...
    def test_joint_connection(self):
        ...
        self.assertPart(
            result.part,
            "825e732dc0cd32fd07ff1b13283b0ad545ce2faf6709bcb519d771ca2a991851",
            "test_joint_connection",
        )

    def test_flat_object_bbox_joint(self):
        """Verify that a flat part correctly generates a 3D bbox and allows joint placement."""
        with BuildPart() as result:
            ml(
                style(
                    width=2,
            ...
            Marker(result.part.bbox_joint(Axis.Y), mat=mat.red)
        self.assertPart(
            result.part,
            "68af8644a9f55df0b453250f1f4d5459b14b82bdf1efdab2d75096b21dc986f4",
            "test_flat_object_bbox_joint",
        )

    def test_bbox_joint_alignment_across_multiple_axes(self):
        """Verify that bounding box joints correctly align and connect components across various directional axes."""
        with BuildPart() as result:
            # Step 1: Create a temporary reference box geometry inside a private context
            with BuildPart(mode=Mode.PRIVATE) as box:
                Box(1, 2, 1)

            # Step 2: Add the initial base box instance into the active build environment
            add(box, mat=mat.red)

            # Step 3: Attach and position subsequent boxes using different directional bounding box joints
            # Connect along the X and negative X axes
            box.part.bbox_joint(Axis.X).to(
                result.part.bbox_joint(-Axis.X), twist=0, mode=Mode.JOIN, mat=mat.green
            )

            # Connect along the positive X axes
            box.part.bbox_joint(Axis.X).to(
                result.part.bbox_joint(Axis.X), twist=0, mode=Mode.JOIN, mat=mat.blue
            )

            # Connect along the Y and negative Y axes
            box.part.bbox_joint(Axis.Y).to(
                result.part.bbox_joint(-Axis.Y), twist=0, mode=Mode.JOIN, mat=mat.yellow
            )

        self.assertPart(
            result.part,
            "c0a5dba5cf56c17e805e640a27392a93e4250f3b7c8344cb204426b217c0639a",
            "test_bbox_joint_alignment_across_multiple_axes",
            use_materials=True,
        )

    def test_bend_operation_deformable_joints(self):
        """Verify that joints with deformable=True correctly follow geometry during a bend operation."""
        with BuildPart() as result:
            # Create a thin plate base geometry
            Box(2, 1, 0.1)

            # Select target faces for subdivision
            sel_faces = faces().bottom() + faces().top()

            # Define joints that must account for geometry deformation
            j_x_pos = result.part.bbox_joint(Axis.X, deformable=True)
            j_x_neg = result.part.bbox_joint(-Axis.X, deformable=True)
            j_y = result.part.bbox_joint(Axis.Y, deformable=True)

            # Subdivide selected faces to allow smooth deformation
            subdivide(faces=sel_faces, cuts=4)

            # Apply a 180-degree bend along the Y axis
            bend(angle=180, axis=Axis.Y)

            # Place visual markers at the updated joint locations
            Marker(j_x_pos.loc, mat=mat.blue)
            Marker(j_x_neg.loc, mat=mat.red)
            Marker(j_y.loc, mat=mat.green)

        # Verify the generated part against the expected regression hash
        self.assertPart(
            result.part,
            "239817ecda5b3f95dcae97bef40c5bb2d951f20f3de02168af081fc45532a5d5",
            "test_bend_operation_deformable_joints",
            use_materials=True,
        )

    def test_joint_registration_and_propagation(self):
        """Verify joint registration, propagation during assembly, and multi-joint connection."""
        with BuildPart() as result:
            # Create the base part
            Box(1, 1, 0.2)
            # Register a joint on the top face of the base part
            result.part.register_joint(
                "top", result.part.joint(faces().top()[0].at(uv))
            )
            # Apply local coordinate transformation to the base part
            result.loc *= Pos(X=1, Y=2, Z=-3) * Rot(X=30, Y=40, Z=50)

            with BuildPart(mode=Mode.PRIVATE) as child:
                # Create the child part to be attached
                Cone(0.2, 0.1, 1)
                # Register a top joint on the child with propagation enabled
                child.part.register_joint(
                    "child_top",
                    child.part.joint(faces().top()[0].at(uv)),
                    propagate=True,
                )
                # Register a bottom joint on the child without propagation
                child.part.register_joint(
                    "child_bottom", child.part.joint(faces().bottom()[0].at(uv))
                )

            # Connect child's bottom joint to the base part's top joint (triggers propagation for child_top)
            child.part.joint_by_name("child_bottom").to(
                result.part.joint_by_name("top")
            )
            # Verify the propagated child_top joint can be used for a subsequent connection
            child.part.joint_by_name("child_top").to(
                result.part.joint_by_name("child_top")
            )

        # Assert the final combined geometry hash to ensure correct transformation and assembly
        self.assertPart(
            result.part,
            "22816509e7e66002b63fe8c6c5f0fdbcbdab937bc9e23f5310003f5994c3716a",
            "test_joint_registration_and_propagation",
        )
