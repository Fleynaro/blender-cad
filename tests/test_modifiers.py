...
class TestGeometricModifiers(BaseCADTest):
    ...
    def test_vertex_transform(self):
        ...
        self.assertPart(
            result.part,
            "f49050e93fef5dfe65dfc2bb9f457e95c5f63537e7ba2ced913514ed6adc2ff1",
    ...
    def test_global_vertex_transform(self):
        ...
        self.assertPart(
            result.part,
            "e81ce80004306273739a1c596c6b881798eaca95c907e124f725799eedef718c",
            "test_global_vertex_transform",
        )
    ...
    def test_proportional_linear_transform_and_extrude(self):
        ...
        with BuildPart() as result:
            ...
            extrude(
                faces().top(),
                op=Pos(Z=1) * Scale(XY=2),
                prop_edit=LinearPropEdit(Axis.Z),
            )
    ...
    def test_proportional_math_operations_on_extrude(self):
        ...
        with BuildPart() as result:
            ...
            math_prop = (
                LinearPropEdit(Axis.X) + LinearPropEdit(Axis.Y).clamp(0.5, 0.8) - 0.1
            ) / 2
    ...
    def test_edge_selection_and_bevel(self):
        ...
        self.assertPart(
            result.part,
            "6165eae68ec63c13703f2ee3a97f30de72fb1916482b1e1fc77cc8434504e174",
            "test_edge_selection_and_bevel",
        )
    ...
    def test_wrap_operation(self):
        ...
        with BuildPart() as result:
            ...
            wrap(
                cyl, loc=cyl.faces().cylinders_only()[0].at(uv), segments=4, offset=0.01
            )
