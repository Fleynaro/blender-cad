...
class TestPart(BaseCADTest):
    ...
    def test_location_update_preserves_scale(self):
        ...
        with BuildPart() as result:
            ...
            self.assertAlmostEqual(
                result.transform.sx, 2.0, 4, "Scale X was lost after moving"
            )
            self.assertAlmostEqual(
                result.transform.sy, 2.0, 4, "Scale Y was lost after moving"
            )
            self.assertAlmostEqual(
                result.transform.sz, 2.0, 4, "Scale Z was lost after moving"
            )
    ...
    def test_set_scale_with_axis_anchor(self):
        ...
        with BuildPart() as result:
            ...
            self.assertEqual(
                result.transform.scale.y, 1.0, "Scale Y should remain unchanged"
            )
            ...
            self.assertEqual(
                result.transform.position.x, -1.0, "Part should have shifted towards -X"
            )
    ...
    def test_set_size_with_axis_anchor(self):
        ...
        with BuildPart() as result:
            ...
            self.assertEqual(
                result.transform.scale.y,
                0.5,
                "Scale X should be 0.5 (half of original)",
            )
            ...
            self.assertEqual(
                ...
                -0.5,
                "Part center should shift to -0.5 to maintain anchor",
            )
    ...
    def test_bbox_part(self):
        ...
        with BuildPart() as result:
            ...
            result.transform = (
                Pos(X=3, Y=4, Z=5) * Rot(X=30, Y=40, Z=50) * Scale(X=0.5, Y=2, Z=4)
            )
    ...
    def test_convex_hull_part(self):
        ...
        with BuildPart() as result:
            ...
            self.assertEqual(
                result.part.convex_hull_part,
                convex_hull,
                "convex_hull_part should be cached",
            )
    ...
    def test_project_2d_mesh_flattening_with_zero_dissolve_limit(self):
        ...
        with BuildPart() as result:
            ...
            chain(
                chain.twist(
                    ml(
                        style(width=2, height=1, background_mat=mat.red, extrude=0.1),
                        ml(
            ...
            add(
                result.part.project_2d(dissolve_limit=0.0),
                offset=Pos(Y=10),
                mode=Mode.JOIN,
            )
        ...
        def build_obj(part: Part | None):
            with Locations(Pos(X=5, Y=6, Z=7) * Rot(X=30, Y=40, Z=50)):
                ...
                with BuildPart(part=part and part.copy(), mode=Mode.JOIN):
                    with BuildPart(
                        part=part and part.copy(), offset=Pos(Z=2), mode=Mode.JOIN
                    ):
